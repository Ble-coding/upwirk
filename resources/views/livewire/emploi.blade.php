<div class="px-3 py-5 mb-3 shadow-sm hover:shadow-md duration-200 rounded border border-gray-200">
    <div class="flex items-center justify-between">
      <h1 class="text-xl font-bold text-green-800">{{ $emploi->title }}</h1>
    ...
    <a href="{{ route('jobs.show', $emploi->id) }}" class="ml-1 text-gray-800 font-semibold hover:text-green-800">Consulter la mission</a>
    </div>
    <div class="flex items-center text-sm text-gray-600 font-semibold mb-2">
      ...
      Ouvert • {{ number_format($emploi->price / 100, 2, ',', ' ') }} €
    </div>
    <span class="block text-sm">Il y a <span class="text-indigo-400">{{ $emploi->proposals->count() }}</span> @choice('proposition|propositions', $emploi->proposals) pour cette mission.</span>
</div>
